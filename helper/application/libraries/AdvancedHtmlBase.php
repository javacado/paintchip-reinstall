...
if (!class_exists('PGBrowser')) {
	...
	class AdvancedHtmlBase {
		...
		public function __call($key, $args) {
			...
			*/
			...
			case 'setattribute':$arg0 = $args[0];
				$arg1 = $args[1];return $this->$arg0 = $arg1;
			...
			if (in_array($key, array('dom', 'node', 'doc'))) {
				return;
			}
	...
	class AHTMLNodeList implements Iterator, Countable, ArrayAccess {
		...
		*/
		...
		public function doMath($nl, $op = 'plus') {
			...
			switch ($op) {
			case 'plus':$new_paths = array_unique(array_merge($paths, $other_paths));
				break;
			case 'minus':$new_paths = array_diff($paths, $other_paths);
				break;
			case 'intersect':$new_paths = array_intersect($paths, $other_paths);
				break;
			}
	...
	class CSS {
		...
		static function do_pseudo($str, $name) {
			...
			default:$inner = self::translate($value);
				break;
			}
...
//     && )
			...
			switch (true) {
			case preg_match('/^(text|comment)$/', $attr, $m): $attr = $m[1] . "()";
				break;
			case !preg_match('/[@(]/', $attr): $attr = '@' . $attr;
				break;
			}
...
//    $value = "'" . preg_replace('/^["\'](.*)["\']$/', '\1', $value) . "'";
		...
		static function translate_part($str, $last_nav = '') {
			...
			foreach (preg_split($re, $str, 0, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY) as $token) {
				switch (true) {
				case preg_match('/^:/', $token): $retval[] = self::do_pseudo($token, $name);
					break;
				case preg_match('/^\[/', $token): $retval[] = self::do_braces($token);
					break;
				case preg_match('/^#/', $token): $retval[] = "[" . self::do_id($token) . "]";
					break;
				case preg_match('/^\./', $token): $retval[] = "[" . self::do_class($token) . "]";
					break;
				default:$name = $token;
				}
			}
